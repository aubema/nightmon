...
def input(argv):
    ...
    Rfile = "undefined"
    try:
        opts, args = getopt.getopt(argv, "h:v:r:ev:er:", ["help=", "vfile=", "rfile=", "extv=", "extr="])
...
limiti = 3.7 # limitting stars magnitude NEED TO BE LOWER THAN 6 but 3.7 is a magic number
limits = 1.2
...
for band, xpoli, ypoli, imagi, imbkg, imstars   in (
    ...
    time=ts.utc(int(datearr[0]), int(datearr[1]), int(datearr[2]), int(datearr[3]), int(datearr[4]), int(datearr[5]))
