...
limit = 2.  # limitting stars magnitude
ds = pd.read_csv('simbad_lt_6Vmag_r1.8.csv',header=0,sep=";")
...
azistar[ altstar < 0 ] = np.nan
magv[ altstar < 0 ] = np.nan
magr[ altstar < 0 ] = np.nan
altstar[ altstar < 0 ] = np.nan
...
plt.plot(azistar,altstar,"or")
#plt.show()

index , pindex = find_close_indices(az, el, azistar , altstar, 0.5)
...
plt.figure()
plt.imshow(falseskyV, cmap="inferno")
plt.colorbar()
plt.title("Stars in V band")

plt.figure()
plt.imshow(falseskyR, cmap="inferno")
plt.colorbar()
plt.title("Stars in R band")
...
# cfrac = clear fraction = 1 - cloudfraction
