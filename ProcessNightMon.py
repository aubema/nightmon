...
for band, xzen, yzen, xpol, ypol, imag, imbkg, imstars, outf in (
    ...
    # background image
    bkg = Background2D(imag, (5, 5), filter_size=(3, 3),
    ...
    elmin = 30
    zemax = 90 - elmin
    sources = sources[np.sqrt((sources['xcentroid']-nx/2)**2+(sources['ycentroid']-ny/2)**2) < np.sqrt((xzen-xpol)**2+(yzen-ypol)**2)*zemax/(90-p["Latitude"])]
